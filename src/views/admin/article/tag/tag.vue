  ...
  <div>
    ...
    >
      ...
      <template #title="{ value, row }">
        <span :style="`color: ${row.color};`">{{ value }}</span>
      </template>
    ...
    <MFormDialog
      ...
      v-bind="formDialog"
      :fields="dialogFields"
      :httpAdd="articleTagAdd"
      :httpEdit="articleTagEdit"
      :httpGet="articleTagDetails"
      @reload="reload"
    ></MFormDialog>
  </div>
</template>
...
const del = (id: number) => {
  articleTagDel(id).then(clearSelectKeysReload)
}
</script>
